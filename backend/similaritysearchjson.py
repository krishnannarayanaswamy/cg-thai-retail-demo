...
def ann_similarity_search():
   ...
   # embeddings = response.embeddings[0]
    ...
    ORDER BY openai_description_embedding ANN OF {embeddings} LIMIT 50;
    """
    )
    ...
    for r in top_products:
        response.append({
            'id': r.product_id,
            'name': r.brand,
            'engdesc': r.short_description_en,
    ...
    #                        "content": "I found these some products I would recommend"})

    #products_list = []

    #for row in response:
        #trans= translator.translate(row.description)
    #    brand_dict = {'role': "assistant", "content": f"{row['id']}, {row['shortdescription']}, {row['name']}, {row['price']}"}
